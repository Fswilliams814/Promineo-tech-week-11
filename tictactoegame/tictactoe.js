...
let playerSymbol = 'X'; // a variable to keep track of the current player
let gameEnded = false; // a boolean variable to keep track of whether the game is over
...
function handleCellClick(event) {
  ...
  cell.innerText = playerSymbol; // set the clicked cell's content to the current player's symbol

  // Check to see who won with an if statement - we will create a function that checks for the win and returns 
  //a boolean value then use the boolean operater from the checkForWin function
  ...
  } else if (checkForDraw()) { // check if the game is a draw - will need to create a function to pass in here
    endGame(true); // call the endGame function with true to indicate there was a draw
  } else {
    // Switch players and update message
    playerSymbol = playerSymbol === 'X' ? 'O' : 'X'; // using a conditional operator to switch the current player
    updateMessage(); // update the message element with the new current player
  }
}

// Function to check for winning conditions - this will return a boolean value
